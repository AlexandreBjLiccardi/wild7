...
import static fr.wild.common.IoFileSystem.file_asString;
...
// *** [Généré automatiquement] Ecrivez vos propres variables globales ici. ***
	protected String DVP_result_processLine ; // Ligne contenant le résultat du traitement xQuery
...
//	//	//	Etape	"1" : poids relatif de 10, Contrôle de l'objet
	...
	// *** [Généré automatiquement] Ecrivez votre code JAVA ici. ***
		...
		Transformer transformer = tFactory.newTransformer(new StreamSource(this.pathToFile));
//	//	//	Etape	"2" : poids relatif de 100, Application de la transformation (+++)
		this.WILD_setStep(); // Ne pas modifier
	// *** [Généré automatiquement] Ecrivez votre code JAVA ici. ***
		String newOutPutPath = WILD_convertPath(i_fileOutput);
		File tOutput = new File(newOutPutPath);
		if(tOutput.exists())tOutput.delete();
		transformer.transform(new StreamSource(xmlFileToProcess), new StreamResult(tOutput));
		DVP_result_processLine =  file_asString(newOutPutPath);
//	//	//	Etape	"3" : poids relatif de 10, Contrôle des flux
		this.WILD_setStep(); // Ne pas modifier
	// *** [Généré automatiquement] Ecrivez votre code JAVA ici. ***
		WILD_toReturn = newOutPutPath ;
//	//	//	Output	"transform_output1" : Nom du fichier de sortie (String)
		this.WILD_setOutput("transform_output1",newOutPutPath); // Ne pas modifier
...
/**
 * Récupération du résultat des traitements, sous forme d'une chaîne de caractère unique
 * @return Résultat du dernier traitement réalisé
 * @throws Exception
 */
public String DVP_getResult() throws Exception{ 
